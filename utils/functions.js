...
module.exports = async client => {
	...
	client.createGuild = async g => {
		...
		if (client.isEmpty(data)) {
			const merged = Object.assign(
				...
				{guildID: g.id, guildName: g.name, ownerId: g.ownerId}
			);
			const createGuild = await new Guild(merged);
			return createGuild
				.save()
				.then(g => client.logger(`${client.timestampParser()} => Nouveau serveur => ${g.guildName}`));
		}
	};

	client.deleteGuild = async g => {
		await Guild.deleteOne({guildID: g.id})
			.exec()
			.then(() => client.logger(`${client.timestampParser()} => Suppression d'un serveur => ${g.name}`));
	};
	//User
	...
	client.deleteUser = async user => {
		await User.deleteOne({userID: user.userID})
			.exec()
			.then(() => client.logger(`${client.timestampParser()} => Utilisateur suprimée => ${user.pseudo}`));
	};
	...
	client.getPercent = (my, myC, sur) => {
		const percent = my / myC;
		if (percent < 1) {
			let per = 1 - percent;
			per = per.toFixed(2);
			const result = `⚠️ | **${per.split('.')[1]}% en dessous de la moyenne de classe** | ⚠️`;
			return result;
		} else if (percent > 1) {
			const per = percent.toFixed(2).toString();
			const result = `✅ | **${per.split('.')[1]}% au-dessus de la moyenne de classe** | ✅`;
			return result;
		}
	};
	...
	const updateStats = async (type, value, user, docs, guildId) => {
		...
		switch (type) {
			case 'cmd':
				...
				await docs
					.save()
					.then(() =>
						client.logger(`${client.timestampParser()} => [STATS] Commande => ${value} => (${user.tag})`)
					);
				break;
			case 'dm':
				docs.dms.push({
				...
				await docs
					.save()
					.then(() =>
						client.logger(`${client.timestampParser()} => [STATS] DM => ${value} => (${user.tag})`)
					);
				break;
			default:
				break;
	...
	client.makeOrUpdateStats = async (type, value, user, guildId) => {
		const stats = await Stats.find().catch(() => {});
		if (client.isEmpty(stats)) {
			return client.createStat().then(docs => {
				return updateStats(type, value, user, docs, guildId);
			});
		} else {
			return updateStats(type, value, user, stats[stats.length - 1], guildId);
		}
	};

	client.addOrUpdateGuild = async (g, type, value, user) => {
		if (client.isEmpty(await client.getGuild(g))) client.createGuild(g);
		else client.updateGuild(g);
		return client.makeOrUpdateStats(type, value, user, g.id);
	};
	...
	client.timestampParser = num => {
		if (num) {
			...
			let date = new Date(Date.now()).toLocaleString('fr-FR', {
	...
	client.addHomeworkToNotion = async (name, content, startDate, endDate) => {
		try {
			await notion.pages
				...
				.then(res => {
					client.logger(
						`${client.timestampParser()} => Homework added to notion --> ${res.properties['Project name']
